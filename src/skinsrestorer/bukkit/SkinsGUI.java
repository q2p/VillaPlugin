...
public class SkinsGUI extends ItemStack implements org.bukkit.event.Listener
{
  ...
  {
    Inventory inventory = org.bukkit.Bukkit.createInventory(null, 54, "§9Skins Menu - Page " + page);
    ...
    for (String s : skinsList.keySet())
      if (skinsList.get(s) != null)
      {
        ...
        if (inventory.firstEmpty() == -1) {
          ...
          inventory.addItem(new ItemStack[] { createSkull(skinsList.get(s), s) });
        }
      }
    return inventory;
  }
  ...
  private static ItemStack setSkin(ItemStack head, String b64stringtexture) {
    ...
    try {
      skinsrestorer.shared.utils.ReflectionUtil.getField(headMetaClass, "profile", GameProfile.class, 0).set(headMeta, profile);
  ...
  public void onCLick(InventoryClickEvent e) {
    ...
    if (e.getCurrentItem() != null) {
      ...
      if (e.getCurrentItem().getType() == Material.SKULL_ITEM) {
        ...
        int currentPage = ((Integer)getMenus().get(player.getName())).intValue();
        getMenus().put(player.getName(), Integer.valueOf(currentPage + 1));
