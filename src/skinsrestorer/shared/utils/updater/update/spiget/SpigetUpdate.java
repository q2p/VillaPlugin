...
import skinsrestorer.shared.utils.updater.update.spiget.download.DownloadCallback;
import skinsrestorer.shared.utils.updater.update.spiget.download.UpdateDownloader;
  ...
  extends SpigetUpdateAbstract
{
  ...
  public boolean downloadUpdate() {
    ...
    boolean allowExternalDownload = (properties != null) && (properties.containsKey("externalDownloads")) && (Boolean.valueOf(properties.getProperty("externalDownloads")).booleanValue());
  ...
  {
    if (!(plugin instanceof JavaPlugin)) return null;
    try {
      Method method = JavaPlugin.class.getDeclaredMethod("getFile", new Class[0]);
      method.setAccessible(true);
      return (File)method.invoke(plugin, new Object[0]);
